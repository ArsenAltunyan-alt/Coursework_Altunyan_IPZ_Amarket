...
def _generate_description_from_title(title):
    ...
    prompt = f"""
        Створи короткий опис оголошення українською для: "{title}".

        Структура (ОБОВʼЯЗКОВО):
        - 1 речення: що це за товар або послуга
        - 2–3 речення: загальний опис
        - Далі з нового рядка список з 3–5 параметрів

        Формат параметрів (СТРОГО):
        Назва параметра: [вкажіть параметр]

        Правила:
        - Без реклами та оцінних суджень
        - Не вигадуй жодних чисел або характеристик
        - Обирай параметри залежно від типу оголошення
        - Загальний обсяг: ДО 90 слів
        """
